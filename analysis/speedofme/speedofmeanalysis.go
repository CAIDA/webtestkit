...
func SpeedPlotRRTime(filename string, metatiming *SpeedMetaTiming, measflow map[string]*SpeedMeasFlow) {
	n := len(measflow)
	...
	for _, flow := range measflow {
		if flow.DownUp == 0 {
			...
			if flow.XhrTiming.Header > 0 && flow.XhrTiming.Opened > 0 && flow.PcapInfo.Response != nil && flow.PcapInfo.Request != nil {
				...
				if flow.PcapInfo.Response.Ts > 0 && flow.PcapInfo.Request.Ts > 0 {
					log.Println("flow", float64(flow.XhrTiming.Header-flow.XhrTiming.Opened)/1000000-float64(flow.PcapInfo.Response.Ts-flow.PcapInfo.Request.Ts), float64(flow.XhrTiming.Done-flow.XhrTiming.Opened)/1000000, float64(flow.Rrs.RrsTimeline[len(flow.Rrs.RrsTimeline)-1].Ts-flow.Rrs.RrsTimeline[0].Ts))
...
func SpeedPlotFlow(filename string, ct SpeedTest) {
	metatiming := ct.Meta
	measflow := ct.Flowmap
	p := plot.New()
	pr := plot.New()
...
func SpeedPlotUpload(filename string, ct SpeedTest) {
	metatiming := ct.Meta
	measflow := ct.Flowmap
	p := plot.New()
	...
	_ = upcsv.NewCSV(common.Getfilename(filename) + ".up.csv")
	for _, flow := range measflow {
		if flow.DownUp == 1 {
			log.Println(flow.ReqId, flow.Rrs, flow.XhrTiming)
			...
			for _, xdata := range ap {
				tmpdata := []string{strconv.FormatFloat(xdata.X, 'f', -1, 64), flow.Host, flow.Port, flow.SrcPort, "1", strconv.FormatFloat(xdata.Y, 'f', -1, 64)}
				upcsv.AddOneToCSV(tmpdata)
			}
...
func OutputStat(measflow map[string]*SpeedMeasFlow) {
